                      ...
                      const int reclen)               // The length of a tuple in the result relation
{
	...
	while(rightIsGood == OK && leftIsGood == OK){
		...
		if (compare > 0){ //right key < left key
			rightIsGood = right.next(rightRec);
		}
		else if (compare < 0){ //left key < right key
			leftIsGood = left.next(leftRec);
		}
		else { // left key == right key
			
			//put left U right projected into result record
			newRec.length = 0;
			...
			/* output further tuples that match w/ right */
			Status rMark = right.setMark();
			Status lMark = left.setMark();
			Record leftRec2, rightRec2;
			Status left2_good, right2_good;
			
			left2_good = left.next(leftRec2);
			while (left2_good == OK && matchRec(leftRec2, rightRec, attrDesc1, attrDesc2) == 0){
				
				/* left2 U right projected into result record */
				newRec.length = 0;
				for (int i = 0; i < projCnt; i++){
					if (attrDescArray[i].relName == attrDesc1.relName){
						memcpy((char*)newRec.data + newRec.length, (char*)leftRec2.data+attrDescArray[i].attrOffset, attrDescArray[i].attrLen);
					}
					else {
						memcpy((char*)newRec.data + newRec.length, (char*)rightRec.data+attrDescArray[i].attrOffset, attrDescArray[i].attrLen);
					}
			...
			while (right2_good == OK && matchRec(leftRec, rightRec2, attrDesc1, attrDesc2) == 0){
				...
				for (int i = 0; i < projCnt; i++){
					if (attrDescArray[i].relName == attrDesc1.relName){
						...
						memcpy((char*)newRec.data + newRec.length, (char*)rightRec2.data+attrDescArray[i].attrOffset, attrDescArray[i].attrLen);
					}
					newRec.length += attrDescArray[i].attrLen;
				}
				//insert projected record into result
			...
			leftIsGood = left.gotoMark();
			if (rightIsGood != OK || leftIsGood != OK){
				if (rightIsGood != OK)
					return rightIsGood;
				else
					return leftIsGood;
